...
function ContactViewModel() {
	...
	// add contact
     self.addContact = function() {
      if(self.funame() == '' && self.fphone()=='' && self.femail()==''){

      } else {
          self.orderNum(self.orderNum()+1);
          self.contacts.push(new contactRow(self.orderNum(), self.funame(), self.fphone(), self.femail()));
          this.funame("");  this.fphone(""); this.femail("");
      }
     
    }
    
    // remove contact
    self.removeContact = function(contactRow) { self.contacts.remove(contactRow); }

    // rafraichir form
    self.refreshContact = function() { 
      this.funame("");
      this.fphone("");
      this.femail("");
    }
